...
const ssl = process.env.NODE_ENV === "production" ? { rejectUnauthorized: false } : {};

const config = {
    connectionString: `postgres://${user}:${password}@${host}/${database}${process.env.DB_SSL === "true" ? "?ssl=true" : ""}`,
    max: 30,
};
